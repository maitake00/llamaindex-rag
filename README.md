...
- **RAGFlowとの違い**: RAGFlowは中国語前提のトークナイザーで日本語が壊れた。ここではベクトル検索主体にして根本回避。
- **軽量設計**: 重い推論(生成・埋め込み・リランク)は全部既存サービス(Ollama/Xinference)に委譲。Python側はオーケストレーションのみ。
- 「多言語RAGパイプラインを自前設計・実装した」という、案件・記事で映える実績になる。
